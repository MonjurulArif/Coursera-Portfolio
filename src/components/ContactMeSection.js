...
const ContactMeSection = () => {
  ...
  // Showing alert when the form is submitted successfully
  useEffect(() => {
    if (response) {
      onOpen(response.type, response.message);
      // Resetting form if the response is successful
      ...
      <VStack w="1024px" p={32} alignItems="flex-start">
        ...
        <Box p={6} rounded="md" w="100%">
          ...
          <form onSubmit={formik.handleSubmit}>
            <VStack spacing={4}>
              {/* Showing error messages for each field when the field is touched and the validation fails */}
              ...
              <FormControl isInvalid={!!formik.errors.email && formik.touched.email}>
                ...
                <Input
                  ...
                  name="email"
                  type="email"
                  {...formik.getFieldProps("email")} // Input components from Chakra UI controlled components
                />
                {/* Showing error messages for each field when the field is touched and the validation fails */}
              ...
              <FormControl>
                ...
                <Select
                  id="type"
                  name="type"
                  {...formik.getFieldProps("type")} // Input components from Chakra UI controlled components
                >
